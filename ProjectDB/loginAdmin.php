<?php  
	...
	if(isset($_POST['Submit'])){
		...
		if(!$db){
			...
			$nm = $_POST['Email'];
			$ps = $_POST['Password'];
			$query1 = 'SELECT id, password FROM users WHERE email = "' . $nm . '" AND id IN (SELECT admin_id FROM admins)';
			$result = mysqli_query($db, $query1);
			$pass = mysqli_fetch_all($result);
			if(sizeof($pass)){
				if(password_verify($ps, $pass[0][1])){
					header("Location: adminInfo.php?ID=" . $pass[0][0]);
				}
				else{
...
<center>
	...
	<tr>
  	...
  	<td><input type="text" name="Email" size="12" maxlength="80" required/></td>
	</tr>
	...
	<button type="submit" formaction="loginAdmin.php" name="Submit">Login</button>
